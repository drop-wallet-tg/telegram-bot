...
const useCreateImplicitAccount = () => {
  ...
  const createAccount = async () => {
    ...
    const accountId = `${uuid()}.${process.env.NEXT_PUBLIC_NETWORK_ID =="mainnet" ? process.env.NEXT_PUBLIC_RELAYER_ACCOUNT_ID_NEAR_MAINNET :process.env.NEXT_PUBLIC_RELAYER_ACCOUNT_ID_NEAR_TESTNET}`;
